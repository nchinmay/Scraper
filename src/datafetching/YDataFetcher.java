...
public class YDataFetcher
{
	...
	public static InputStream getYQXMLData(final Set<String> symbols) throws Exception
	{
	...
	public static void parseYQXMLQuery(InputStream inputStream) throws Exception
	{
		if (inputStream != null)
		{
			...
			if (quoteNodes != null && quoteNodes.getLength() > 0)
			{
				for (int i = 0; i < quoteNodes.getLength(); ++i)
				{
					...
					CsvFileHelper.writeAsCsvFile(RunHelper.getTodayDataDirectory(), data.getRowKey() + ".csv", ',', data);
					// Cap'n Proto data
	...
	public static MessageBuilder parseYDXMLQuoteToCapnp(Element quote)
	{
		...
		try
		{
			SerializePacked.writeToUnbuffered((new FileOutputStream(RunHelper.getTodayDataDirectory() + db.getSymbol() + ".data")).getChannel(), message);
		}
		catch (IOException e)
		{
			e.printStackTrace();
