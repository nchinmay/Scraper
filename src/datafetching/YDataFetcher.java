...
public class YDataFetcher {
	...
	public static void getYahooData(final Set<String> symbols) {
		if (symbols != null) {
			...
			while (iter.hasNext()) {
				...
				if (symbolsForThisQuery.size() == SYMBOLS_PER_QUERY
						|| !iter.hasNext()) {
					try {
						for (int tries = 0; tries < TRIES_PER_SYMBOL
								&& !symbolsForThisQuery.isEmpty(); ++tries) {
							InputStream is = YDataFetcher
									.getYQXMLInputStream(symbolsForThisQuery);
							Set<String> processedSymbols = YDataFetcher
									.parseYQXMLQueryAndReturnProcessedSymbols(is);
	...
	public static InputStream getYQXMLInputStream(final Set<String> symbols)
			throws Exception {
	...
	public static Set<String> parseYQXMLQueryAndReturnProcessedSymbols(
			...
			if (quoteNodes != null && quoteNodes.getLength() > 0) {
				...
				for (int i = 0; i < quoteNodes.getLength(); ++i) {
					...
					CsvFileHelper.writeAsCsvFile(
							RunHelper.getTodayRunDataDirectory(),
							data.getRowKey() + ".csv", ',', data);
	...
	public static YData parseYDXMLQuote(Element quote) {
		...
		d.setFiftydayMovingAverage(convertToDouble(getValueFromXML(quote,
				"FiftydayMovingAverage")));
		d.setChangeFromFiftydayMovingAverage(convertToDouble(getValueFromXML(
				quote, "ChangeFromFiftydayMovingAverage")));
		d.setTwoHundreddayMovingAverage(convertToDouble(getValueFromXML(quote,
				"TwoHundreddayMovingAverage")));
		d.setChangeFromTwoHundreddayMovingAverage(convertToDouble(getValueFromXML(
				quote, "ChangeFromTwoHundreddayMovingAverage")));
		d.setAverageDailyVolume(convertToLong(getValueFromXML(quote,
				"AverageDailyVolume")));
		...
		d.setMarketCapitalization(convertToDouble(getValueFromXML(quote,
				"MarketCapitalization")));

		d.setEarningsShare(convertToDouble(getValueFromXML(quote,
				"EarningsShare")));
		d.setEPSEstimateCurrentYear(convertToDouble(getValueFromXML(quote,
				"EPSEstimateCurrentYear")));
		d.setEPSEstimateNextYear(convertToDouble(getValueFromXML(quote,
				"EPSEstimateNextYear")));
		d.setEPSEstimateNextQuarter(convertToDouble(getValueFromXML(quote,
				"EPSEstimateNextQuarter")));
		d.setDividendShare(convertToDouble(getValueFromXML(quote,
				"DividendShare")));
		d.setDividendYield(convertToDouble(getValueFromXML(quote,
				"DividendYield")));
		d.setExDividendDate(getValueFromXML(quote, "ExDividendDate"));
		d.setPriceEPSEstimateCurrentYear(convertToDouble(getValueFromXML(quote,
				"PriceEPSEstimateCurrentYear")));
		d.setPriceEPSEstimateNextYear(convertToDouble(getValueFromXML(quote,
				"PriceEPSEstimatenexttYear")));
		d.setOneyrTargetPrice(convertToDouble(getValueFromXML(quote,
				"OneyrTargetPrice")));
		return d;
	}
	...
	public static MessageBuilder parseYDXMLQuoteToCapnp(Element quote) {
		...
		datafetching.YDataCP.YData.Builder db = message
				.initRoot(datafetching.YDataCP.YData.factory);
		...
		db.setFiftydayMovingAverage(convertToDouble(getValueFromXML(quote,
				"FiftydayMovingAverage")));
		db.setChangeFromFiftydayMovingAverage(convertToDouble(getValueFromXML(
				quote, "ChangeFromFiftydayMovingAverage")));
		db.setTwoHundreddayMovingAverage(convertToDouble(getValueFromXML(quote,
				"TwoHundreddayMovingAverage")));
		db.setChangeFromTwoHundreddayMovingAverage(convertToDouble(getValueFromXML(
				quote, "ChangeFromTwoHundreddayMovingAverage")));
		db.setAverageDailyVolume(convertToLong(getValueFromXML(quote,
				"AverageDailyVolume")));
		...
		db.setMarketCapitalization(convertToDouble(getValueFromXML(quote,
				"MarketCapitalization")));

		db.setEarningsShare(convertToDouble(getValueFromXML(quote,
				"EarningsShare")));
		db.setEPSEstimateCurrentYear(convertToDouble(getValueFromXML(quote,
				"EPSEstimateCurrentYear")));
		db.setEPSEstimateNextYear(convertToDouble(getValueFromXML(quote,
				"EPSEstimateNextYear")));
		db.setEPSEstimateNextQuarter(convertToDouble(getValueFromXML(quote,
				"EPSEstimateNextQuarter")));
		db.setDividendShare(convertToDouble(getValueFromXML(quote,
				"DividendShare")));
		db.setDividendYield(convertToDouble(getValueFromXML(quote,
				"DividendYield")));
		db.setExDividendDate(getValueFromXML(quote, "ExDividendDate"));
		db.setPriceEPSEstimateCurrentYear(convertToDouble(getValueFromXML(
				quote, "PriceEPSEstimateCurrentYear")));
		db.setPriceEPSEstimateNextYear(convertToDouble(getValueFromXML(quote,
				"PriceEPSEstimatenexttYear")));
		db.setOneyrTargetPrice(convertToDouble(getValueFromXML(quote,
				"OneyrTargetPrice")));

		try {
			SerializePacked.writeToUnbuffered(
					(new FileOutputStream(RunHelper.getTodayRunDataDirectory()
							+ db.getSymbol() + ".data")).getChannel(), message);
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public static Set<String> readSymbolsFromFile() throws Exception {
		BufferedReader br = new BufferedReader(
				new FileReader(RunHelper.getCurrentRunDirectory()
						+ SP500_FEB_2015_SYMBOL_FILE));
		Set<String> symbols = new HashSet<String>();
